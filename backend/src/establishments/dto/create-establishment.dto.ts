...
import { IsEmail, IsEnum, IsNotEmpty, IsNumber, IsOptional, IsString, Length,MaxLength, ValidateNested } from "class-validator";
import { CreateSupplierAttributesDto } from "src/suppliers/dto/create-supplier-attributes.dto";
import { Type } from "class-transformer";

export class CreateEstablishmentDto {

    @ApiProperty({ enum: EstablishmentType, example: EstablishmentType.RESTAURANT })
    ...
    @ApiProperty({ example: 'A popular restaurant in the heart of the city.', required: false })
    @IsString()
    @IsOptional()
    description?: string;
    ...
    @ApiProperty({ 
        ...
        required: false,
        description: 'Supplier attributes for the establishment (if establishment is a supplier).'
    })
    @IsOptional()
    @ValidateNested()
    @Type(() => CreateSupplierAttributesDto)
