    ...
    <div className="gap-2 flex flex-col">
      <div className="flex flex-row gap-8 items-start justify-between">
        <div className="flex flex-col lg:flex-row gap-2 lg:gap-6 items-start lg:items-center">
          ...
          {supplier.productCategories.map((id) => (
            <span
              key={id}
              className="bg-black/90 text-white text-sm px-2 py-1 rounded-full"
            >
        ...
        <div className="flex flex-row gap-1 items-center">
          ...
          <button
            ...
            className="border-muted border-1 p-2 rounded-full w-full h-max h-full items-center justify-center cursor-pointer group/heart transition-color duration-300 hover:bg-border/50"
          >
            <Heart
              size={18}
      ...
      <div className="flex flex-row gap-2">
        {supplier.labels.map((id) => (
          <Badge key={id}>
            {allLabels.find((l) => l.id === id)?.name ?? id}
          </Badge>
        ))}
      </div>
    </div>
