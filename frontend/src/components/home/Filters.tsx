...
export default function Filters({filters, onChange}: FiltersProps) {
    ...
    function handleChangeType(supplierType: string, checked: boolean) {
        if (checked) {
            onChange({...filters, supplierTypes: [...filters.supplierTypes, supplierType]});
        } else {
            onChange({...filters, supplierTypes: [...filters.supplierTypes.filter((t) => (t) !== supplierType)]})
        }
    }
        ...
        <div className="flex flex-col gap-6 w-1.5/6">
            ...
            <div className="flex flex-col gap-3">
                    ...
                    {CATEGORIES.map((category) => (
                        <Field key={category} orientation="horizontal">
                            <Checkbox
                                ...
                                onCheckedChange={(checked) => handleChangeCategory(category, checked as boolean)}
                                id={category}
                                name={category}
                            />
                            <FieldLabel htmlFor={category} className="font-normal text-sm">{category}</FieldLabel>
                        </Field>
                    ))}
                </div>
                <Separator />
                    ...
                    {LABELS.map((label) => (
                        <Field key={label} orientation="horizontal">
                            <Checkbox
                                ...
                                onCheckedChange={(checked) => handleChangeLabel(label, checked as boolean)}
                                id={label}
                                name={label}
                            />
                            <FieldLabel className="font-normal text-sm" htmlFor={label}>{label}</FieldLabel>
                        </Field>
                    ))}
                </div>
                <Separator />
                <div className="flex flex-col gap-1">
                    ...
                    >
                        {RATINGS.map((rating) => (
                            <div key={rating.label} className="flex flex-row items-center gap-1">
                                <RadioGroupItem value={String(rating.value)} id={rating.label} />
                                <label htmlFor={rating.label} className="font-normal text-sm">{rating.label}</label>
                            </div>
                        ))}
                    </RadioGroup>
                </div>
                    ...
                    {TYPES.map((supplierType) => (
                        <Field key={supplierType} orientation="horizontal">
                            <Checkbox
                                ...
                                onCheckedChange={(checked) => handleChangeType(supplierType, checked as boolean)}
                                id={supplierType}
                                name={supplierType}
                            />
                            <FieldLabel htmlFor={supplierType} className="font-normal text-sm">{supplierType}</FieldLabel>
                        </Field>
                    ))}
                </div>
                <Separator />
                <div className="flex flex-row items-center gap-1">
                    ...
                    <Switch
                        ...
                        onCheckedChange={(checked) => onChange({...filters, isPremium: checked})}
                     />
                </div>
