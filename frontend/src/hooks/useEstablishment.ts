import { initialErrors, initialValues } from "@/types/profile";
import { useEffect, useState } from "react";
import { useAuth } from "./useAuth";
import api from "@/api/axiosConfig";
import type { FieldErrorsType } from "@/types/profile";
...
export default function useEstablishment() {
    ...
    const [fieldErrors, setFieldErrors] = useState<FieldErrorsType>(initialErrors);
    ...
    useEffect(() => {
      ...
      async function getCurrentValues() {
        try {
          ...
          setError('Erreur lors du chargement des informations de l\'établissement. Veuillez réessayer plus tard.');
        } finally {
          setLoading(false);
        }
      };
    ...
    async function patchEstablishment(fieldName: string, value: string) {
      try {
        ...
        setFieldErrors({ ...fieldErrors, [fieldName]: error.response?.data.message[0] ?? null });
        setEditionValues(prev => ({ ...prev, [fieldName]: apiValues[fieldName as keyof typeof apiValues] }));
      }
    }

    return {editionValues, setEditionValues, apiValues, error, loading, patchEstablishment, fieldErrors}
}
